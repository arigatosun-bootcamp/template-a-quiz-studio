...
| デプロイ先 | Vercel（予定） | Webサーバーのホスティング |
| テスト | Vitest | 自動テスト |
...
| 画面 | URL | 説明 |
|------|-----|------|
| ログイン | `/login` | メール＆パスワード、またはGoogleアカウントでログイン |
...
**Googleアカウントの場合:**
- 「Googleでログイン」ボタンを押すだけで登録＆ログインが完了します
- 同じメールアドレスでメール登録とGoogle登録をした場合は自動で統合されます
...
本アプリはGoogle Gemini API（Gemini 2.0 Flash モデル）を使ってクイズを自動生成しています。
...
Gemini 2.0 Flash の料金（2025年2月時点）:
- 入力: $0.10 / 100万トークン
- 出力: $0.40 / 100万トークン
...
**Q: Googleアカウントとメールアドレスで別々に登録してしまいました。**
A: 同じメールアドレスであれば自動的に同一アカウントとして統合されます。
