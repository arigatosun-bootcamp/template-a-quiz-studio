...
| 項目 | 仕様 |
|------|------|
| リトライ回数 | 最大3回（初回 + リトライ2回 = 合計3回試行） |
| リトライ間隔 | 1回目→2回目: 2秒後、2回目→3回目: 4秒後（倍々で待つ） |
| 1回あたりのAPI応答待ち時間 | 最大10秒（10秒以内に応答がなければタイムアウトとして次のリトライへ） |
| 最悪ケースの合計待ち時間 | 約36秒（10秒×3回 + 待ち時間6秒） |
| 全リトライ失敗時 | 「問題の生成に失敗しました。もう一度お試しください」と表示し再試行ボタンを出す |
| リトライ中の表示 | 「クイズを生成中です...」のローディング表示を継続（リトライ中であることはユーザーに見せない） |
