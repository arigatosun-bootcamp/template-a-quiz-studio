...
> Claude API を使ったクイズ生成、回答保存、セッション管理の API が全て動作する状態にする
...
- [ ] Claude API でクイズ5問が生成され、`quizzes` テーブルに保存される
...
- [ ] Vercel プロジェクトの作成・接続
- [ ] 環境変数の設定（Supabase URL / Key, Claude API Key）
- [ ] 本番ビルド・デプロイの実行
- [ ] 本番環境での動作確認
...
| Day 2 | 要作成 | クイズ生成APIの実装（Claude API連携） | - |
